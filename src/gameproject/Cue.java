	...
	public void drawBack() {
		...
		if (drawnBack == true && drawBack_xPos <= 0 && ballsMoving == false) {
			...
			if (angle > 0 && angle < 90) {
				...
				cue.getSpeed().setY(-Math.sin(angleSp) * power / 2.35);
				cue.getSpeed().setX(-Math.sin(Math.PI / 2 - angleSp) * power / 2.35);
			} else if (angle > 90 && angle < 180) {
				//calculates raa and converts to radians
				angleSp = angle - 90;
				angleSp = Math.toRadians(angleSp);
				//trigonometry to calculate speed
				cue.getSpeed().setX(Math.sin(angleSp) * power / 2.35);
				cue.getSpeed().setY(-Math.sin(Math.PI / 2 - angleSp) * power / 2.35);
